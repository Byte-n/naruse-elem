...
export default class upper_right_flag_dialog extends Component {
        ...
        return(
            <view style={{ ...tradePcOrderTem,  ...(animation ? dialogBox : {}) }}>
                <view style={{ position:'relative'}}>
                    ...
                    <text style={{ ...buyBtnStyle,left: '500rpx' }} onClick={() => {this.buyBtnClick({ btnText:'52/3个月',url:buyLink1,cname:'PC续费落地页B-季度' })}}/>
                    <text style={{ ...buyBtnStyle,left: '705rpx' }} onClick={() => {this.buyBtnClick({ btnText:'99/6个月',url:buyLink2,cname:'PC续费落地页B-半年' })}}/>
                    <text style={{ ...buyBtnStyle,left: '910rpx' }} onClick={() => {this.buyBtnClick({ btnText:'148/12个月',url:buyLink3,cname:'PC续费落地页B-年度'})}}/>
                </view>
                <view>
    ...
    getRetainDialog = () => {
        ...
        return (
        <view style={{ ...tradePcOrderTem,...(animation ? dialogBox : {}) }}>
            <view style={{ position:'relative',   cursor: 'pointer'}} onClick={() => {this.buyBtnClick({ btnText:'天降红包/1个月',url:buyLinkRetain,cname:'PC续费挽留B-年度'})}}>
