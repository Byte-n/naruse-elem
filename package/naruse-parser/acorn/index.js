...
exports.acorn = void 0;
var index_1 = require("../expressionType/index");
// this module is clone from acorn@0.12.0 with some modifications
exports.acorn = {};
exports.acorn.version = "0.12.x";
...
exports.acorn.parse = function (inpt, opts) {
    input = String(inpt);
...
var defaultOptions = exports.acorn.defaultOptions = {
...
exports.acorn.parseExpressionAt = function (inpt, pos, opts) {
    input = String(inpt);
    inputLen = input.length;
    setOptions(opts);
    initTokenState(pos);
...
exports.acorn.tokenize = function (inpt, opts) {
    input = String(inpt);
    inputLen = input.length;
    setOptions(opts);
    initTokenState();
...
for (var kw in keywordTypes)
    exports.acorn.tokTypes["_" + kw] = keywordTypes[kw];
...
function isUseStrict(stmt) {
    return options.ecmaVersion >= 5 && stmt.type === index_1.ExpressionStatement &&
        stmt.expression.type === index_1.Literal && stmt.expression.value === "use strict";
}
...
function toAssignable(node, isBinding) {
    if (options.ecmaVersion >= 6 && node) {
        switch (node.type) {
            case index_1.Identifier:
            case index_1.ObjectPattern:
            case index_1.ArrayPattern:
            case index_1.AssignmentPattern:
                break;
            case index_1.ObjectExpression:
                node.type = index_1.ObjectPattern;
                ...
                break;
            case index_1.ArrayExpression:
                node.type = index_1.ArrayPattern;
                toAssignableList(node.elements, isBinding);
                break;
            case index_1.AssignmentExpression:
                if (node.operator === "=") {
                    node.type = index_1.AssignmentPattern;
                }
                else {
                    raise(node.left.end, "Only '=' operator can be used for specifying default value.");
                }
                break;
            case index_1.MemberExpression:
                if (!isBinding)
                    break;
            default:
...
function toAssignableList(exprList, isBinding) {
    if (exprList.length) {
        ...
        switch (last.type) {
            case index_1.RestElement:
                break;
            case index_1.SpreadElement:
                last.type = index_1.RestElement;
                var arg = last.argument;
                toAssignable(arg, isBinding);
                if (arg.type !== index_1.Identifier && arg.type !== index_1.MemberExpression && arg.type !== index_1.ArrayPattern)
...
function parseSpread(refShorthandDefaultPos) {
    ...
    return finishNode(node, index_1.SpreadElement);
}
function parseRest() {
    var node = startNode();
    next();
    node.argument = tokType === _name || tokType === _bracketL ? parseBindingAtom() : unexpected();
    return finishNode(node, index_1.RestElement);
}
...
function parseBindingAtom() {
    ...
    switch (tokType) {
        ...
        case _bracketL:
            ...
            return finishNode(node, index_1.ArrayPattern);
        case _braceL:
...
function parseMaybeDefault(startPos, left) {
    ...
    return finishNode(node, index_1.AssignmentPattern);
}
...
function parseTopLevel(node) {
    ...
    next();
    return finishNode(node, index_1.Program);
}
var loopLabel = {
    kind: "loop"
}, switchLabel = {
...
function parseStatement(declaration, topLevel) {
    ...
    switch (starttype) {
        ...
        default:
            ...
            if (starttype === _name && expr.type === index_1.Identifier && eat(_colon))
...
function parseBreakContinueStatement(node, keyword) {
    ...
    return finishNode(node, isBreak ? index_1.BreakStatement : index_1.ContinueStatement);
}
function parseDebuggerStatement(node) {
    next();
    semicolon();
    return finishNode(node, index_1.DebuggerStatement);
}
function parseDoStatement(node) {
    ...
    if (options.ecmaVersion >= 6)
        ...
        semicolon();
    return finishNode(node, index_1.DoWhileStatement);
}
...
function parseForStatement(node) {
    ...
    if (tokType === _var || tokType === _let) {
        ...
        finishNode(init, index_1.VariableDeclaration);
...
function parseIfStatement(node) {
    ...
    return finishNode(node, index_1.IfStatement);
}
function parseReturnStatement(node) {
    ...
    return finishNode(node, index_1.ReturnStatement);
}
function parseSwitchStatement(node) {
    ...
    for (var cur, sawDefault; tokType != _braceR;) {
        if (tokType === _case || tokType === _default) {
            ...
            if (cur)
                finishNode(cur, index_1.SwitchCase);
    ...
    next(); // Closing brace
    labels.pop();
    return finishNode(node, index_1.SwitchStatement);
}
function parseThrowStatement(node) {
    ...
    node.argument = parseExpression();
    semicolon();
    return finishNode(node, index_1.ThrowStatement);
}
function parseTryStatement(node) {
    ...
    if (tokType === _catch) {
        ...
        node.handler = finishNode(clause, index_1.CatchClause);
    }
    node.guardedHandlers = empty;
    node.finalizer = eat(_finally) ? parseBlock() : null;
    if (!node.handler && !node.finalizer)
        raise(node.start, "Missing catch or finally clause");
    return finishNode(node, index_1.TryStatement);
}
function parseVarStatement(node, kind) {
    next();
    parseVar(node, false, kind);
    semicolon();
    return finishNode(node, index_1.VariableDeclaration);
}
function parseWhileStatement(node) {
    ...
    node.body = parseStatement(false);
    labels.pop();
    return finishNode(node, index_1.WhileStatement);
}
...
function parseEmptyStatement(node) {
    next();
    return finishNode(node, index_1.EmptyStatement);
}
function parseLabeledStatement(node, maybeName, expr) {
    ...
    labels.pop();
    node.label = expr;
    return finishNode(node, index_1.LabeledStatement);
}
function parseExpressionStatement(node, expr) {
    node.expression = expr;
    semicolon();
    return finishNode(node, index_1.ExpressionStatement);
}
...
function parseBlock(allowStrict) {
    ...
    if (oldStrict === false)
        setStrict(false);
    return finishNode(node, index_1.BlockStatement);
}
...
function parseFor(node, init) {
    ...
    node.body = parseStatement(false);
    labels.pop();
    return finishNode(node, index_1.ForStatement);
}
// Parse a `for`/`in` and `for`/`of` loop, which are almost
// same from parser's perspective.
function parseForIn(node, init) {
    var type = tokType === _in ? index_1.ForInStatement : index_1.ForOfStatement;
    next();
    node.left = init;
...
function parseVar(node, noIn, kind) {
    ...
    for (;;) {
        ...
        node.declarations.push(finishNode(decl, index_1.VariableDeclarator));
        if (!eat(_comma))
            break;
    }
    return node;
...
function parseExpression(noIn, refShorthandDefaultPos) {
    ...
    if (tokType === _comma) {
        ...
        return finishNode(node, index_1.SequenceExpression);
    }
    return expr;
}
...
function parseMaybeAssign(noIn, refShorthandDefaultPos) {
    ...
    if (tokType.isAssign) {
        ...
        return finishNode(node, index_1.AssignmentExpression);
    }
...
function parseMaybeConditional(noIn, refShorthandDefaultPos) {
    ...
    if (eat(_question)) {
        ...
        return finishNode(node, index_1.ConditionalExpression);
    }
    return expr;
}
...
function parseExprOp(left, leftStart, minPrec, noIn) {
    ...
    if (prec != null && (!noIn || tokType !== _in)) {
        if (prec > minPrec) {
            ...
            finishNode(node, (op === _logicalOR || op === _logicalAND) ? index_1.LogicalExpression : index_1.BinaryExpression);
...
function parseMaybeUnary(refShorthandDefaultPos) {
    if (tokType.prefix) {
        ...
        if (update)
            ...
            node.argument.type === index_1.Identifier)
            raise(node.start, "Deleting local variable in strict mode");
        return finishNode(node, update ? index_1.UpdateExpression : index_1.UnaryExpression);
    }
    var start = storeCurrentPos();
    ...
    while (tokType.postfix && !canInsertSemicolon()) {
        ...
        checkLVal(expr);
        next();
        expr = finishNode(node, index_1.UpdateExpression);
    }
    return expr;
}
...
function parseSubscripts(base, start, noCalls) {
    if (eat(_dot)) {
        ...
        node.computed = false;
        return parseSubscripts(finishNode(node, index_1.MemberExpression), start, noCalls);
    }
        ...
        return parseSubscripts(finishNode(node, index_1.CallExpression), start, noCalls);
    }
    else if (tokType === _backQuote) {
        var node = startNodeAt(start);
        node.tag = base;
        node.quasi = parseTemplate();
        return parseSubscripts(finishNode(node, index_1.TaggedTemplateExpression), start, noCalls);
    }
    return base;
}
...
function parseExprAtom(refShorthandDefaultPos) {
    switch (tokType) {
        case _this:
            var node = startNode();
            next();
            return finishNode(node, index_1.ThisExpression);
        case _yield:
            if (inGenerator)
                return parseYield();
        case _name:
        ...
        case _regexp:
            ...
            next();
            return finishNode(node, index_1.Literal);
        case _num:
        case _string:
            var node = startNode();
            node.value = tokVal;
        ...
        case _bracketL:
            ...
            return finishNode(node, index_1.ArrayExpression);
        case _braceL:
...
function parseImportExpression() {
    ...
    return finishNode(node, index_1.ImportExpression);
}
function parseParenAndDistinguishExpression() {
    ...
    if (options.ecmaVersion >= 6) {
        ...
        if (exprList.length > 1) {
            ...
            finishNodeAt(val, index_1.SequenceExpression, innerEnd);
        }
        else {
            val = exprList[0];
        }
    ...
    if (options.preserveParens) {
        ...
        par.expression = val;
        return finishNode(par, index_1.ParenthesizedExpression);
    }
    else {
        return val;
    }
...
function parseNew() {
    ...
    return finishNode(node, index_1.NewExpression);
}
...
function parseTemplateElement() {
    ...
    return finishNode(elem, index_1.TemplateElement);
}
function parseTemplate() {
    ...
    next();
    return finishNode(node, index_1.TemplateLiteral);
}
...
function parseObj(isPattern, refShorthandDefaultPos) {
    ...
    while (!eat(_braceR)) {
        ...
        else if (options.ecmaVersion >= 5 && !prop.computed && prop.key.type === index_1.Identifier &&
        ...
        else if (options.ecmaVersion >= 6 && !prop.computed && prop.key.type === index_1.Identifier) {
            prop.kind = "init";
            if (isPattern) {
    ...
    return finishNode(node, isPattern ? index_1.ObjectPattern : index_1.ObjectExpression);
}
...
function parseFunction(node, isStatement, allowExpressionBody) {
    ...
    return finishNode(node, isStatement ? index_1.FunctionDeclaration : index_1.FunctionExpression);
}
...
function parseMethod(isGenerator) {
    ...
    parseFunctionBody(node, allowExpressionBody);
    return finishNode(node, index_1.FunctionExpression);
}
...
function parseArrowExpression(node, params) {
    ...
    return finishNode(node, index_1.ArrowFunctionExpression);
}
...
function parseClass(node, isStatement) {
    ...
    while (!eat(_braceR)) {
        ...
        if (tokType !== _parenL && !method.computed && method.key.type === index_1.Identifier &&
        ...
        classBody.body.push(finishNode(method, index_1.MethodDefinition));
    }
    node.body = finishNode(classBody, index_1.ClassBody);
    return finishNode(node, isStatement ? index_1.ClassDeclaration : index_1.ClassExpression);
}
...
function parseIdent(liberal) {
    ...
    next();
    return finishNode(node, index_1.Identifier);
}
...
function parseExport(node) {
    ...
    if (eat(_default)) {
        ...
        if (expr.id) {
            switch (expr.type) {
                case index_1.FunctionExpression:
                    expr.type = index_1.FunctionDeclaration;
                    break;
                case index_1.ClassExpression:
                    expr.type = index_1.ClassDeclaration;
                    break;
            }
        }
    ...
    return finishNode(node, index_1.ExportDeclaration);
}
...
function parseExportSpecifiers() {
    ...
    if (tokType === _star) {
        ...
        var node = startNode();
        next();
        nodes.push(finishNode(node, index_1.ExportBatchSpecifier));
    }
    else {
        // export { x, y as z } [from '...']
        expect(_braceL);
        while (!eat(_braceR)) {
            ...
            nodes.push(finishNode(node, index_1.ExportSpecifier));
        }
    }
    return nodes;
}
...
function parseImport(node) {
    ...
    semicolon();
    return finishNode(node, index_1.ImportDeclaration);
}
...
function parseImportSpecifiers() {
    ...
    if (tokType === _name) {
        ...
        node.name = null;
        node['default'] = true;
        nodes.push(finishNode(node, index_1.ImportSpecifier));
        if (!eat(_comma))
            return nodes;
    }
    if (tokType === _star) {
        ...
        nodes.push(finishNode(node, index_1.ImportBatchSpecifier));
        return nodes;
    }
    expect(_braceL);
    while (!eat(_braceR)) {
...
function parseYield() {
    ...
    return finishNode(node, index_1.YieldExpression);
}
exports["default"] = exports.acorn;
