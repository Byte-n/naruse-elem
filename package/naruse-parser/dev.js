...
var evaluate_map = (_a = {},
    ...
    _a[ForStatement] = function (node, scope) {
        for (var new_scope = new Scope('loop', scope), init_val = node.init ? evaluate(node.init, new_scope) : null; node.test ? evaluate(node.test, new_scope) : true; node.update ? evaluate(node.update, new_scope) : void (0)) {
            var result = evaluate({ type: 'BlockStatement', body: node.body }, new_scope);
            if (result === BREAK_SINGAL) {
                break;
            }
    ...
    _a[UpdateExpression] = function (node, scope) {
        ...
        var $var;
        if (node.argument.type === Identifier) {
            var name_6 = node.argument.name;
            $var = scope.$find(name_6);
            if (!$var)
                throw createError(errorMessageList.notYetDefined, name_6, node, thisRunner.source);
        }
    ...
    _a[AssignmentExpression] = function (node, scope) {
        ...
        var left = node.left;
        if (left.type === Identifier) {
            var name_7 = left.name;
            var $var_or_not = scope.$find(name_7);
            if (!$var_or_not)
    ...
    _a[ForInStatement] = function (node, scope, isForOf) {
        ...
        var forInit = function (value) {
            ...
            if (id.type === Identifier) {
                var name_8 = id.name;
                new_scope.$declar(kind, name_8, value);
            }
            else {
...
var Scope = /** @class */ (function () {
    function Scope(type, parent) {
        this.content = {};
        this.prefix = '';
        this.invasived = false; // 标记为侵入式Scope，不用再多构造啦
        this.type = type;
...
function evaluate(node, scope, runner) {
    if (runner)
        thisRunner = runner;
...
var getLineInfo = acorn.getLineInfo = function (input, offset) {
...
var isIdentifierStart = acorn.isIdentifierStart = function (code) {
...
var isIdentifierChar = acorn.isIdentifierChar = function (code) {
