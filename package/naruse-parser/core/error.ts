import { getScopeRunner, Scope } from './scope';
...
export const createError = (msg: ErrorMessageList, value: any, node?: any, scope?: Scope) => {
    ...
    const codeStack = [];
    if (node && source) {
        const cm = buildNodeErrorMessage(scope, node);
        codeStack.push(cm);
        message = `${message} \n 错误代码：${cm}`;
    }
...
export const buildNodeErrorMessage = (scope?: Scope, node?: any) => {
    if (!node || !scope) {
        return '';
    }
    const source = getScopeRunner(scope)?.source;
    ...
    let errorMsg = source.slice(node.start, node.end);
