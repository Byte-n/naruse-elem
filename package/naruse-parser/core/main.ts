...
export class Interpreter {
	...
	protected objectExpressionHandler(node: ESTree.ObjectExpression) {
		...
		(node.properties as any).forEach(property => {
	...
	protected arrayExpressionHandler(node: ESTree.ArrayExpression) {
		//fix: [,,,1,2]
		const items: any = node.elements.map(element =>
	...
	// ... catch(e){...}
	protected catchClauseHandler(node: ESTree.CatchClause | any): (e: Error) => any {
	...
	protected createMemberKeyGetter(node: ESTree.MemberExpression | any): Getter {
