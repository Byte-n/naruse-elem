...
// 一元运算符
const UNARY_OPERATORS = { '-': true, '!': true, '+': true, '=': true };
// 二元运算符
...
// 赋值关键字
const assignKeyWord = ['let', 'const', 'var']
...
class ExpressionParser {
    constructor(expr) {
        if (typeof expr !== 'string') {
            throw new Error(`条件解析需要一个字符串，但是传入的是[${typeof expr}]`);
        }
        this.expr = expr;
        this.lastNodeType = ''
    }
...
console.log(expression.valueOf({ load: 5, qwer: [1], ww: "123" }));
