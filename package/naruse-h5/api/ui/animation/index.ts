...
class Animation implements Naruse.Animation {
  unit: string
  id: number
  ...
  constructor (
    ...
    document.body.addEventListener(TRANSITION_END, (e: TransitionEvent) => {
      ...
      if (animName === `naruse-h5-poly-fill/${this.id}/create-animation`) {
  ...
  scale (x, y) {
    this.transform.push(`scale(${[x,y].filter(Boolean).join(',')})`)
    return this
  }
  ...
  step (arg: Partial<IAnimationAttr> = {}) {
    ...
    return this as Naruse.Animation
  }

  // 创建底层数据
  createAnimationData () {
    const animIndex = `naruse-h5-poly-fill/${this.id}/create-animation__${this.animationMapCount++}`
    // 记录动画分几个 step
  ...
  // 动画数据产出
  export () {
    return this.createAnimationData() as unknown as ReturnType<Naruse.Animation['export']>
  }
}

// h5 的 createAnimation
export const createAnimation: typeof Naruse.createAnimation = (option) => {
