...
const initVersionLogger = (name, version) => {
  console.log(`%c naruse %c ${name} v${version} %c`, 'background:#17c0eb ; padding: 1px; border-radius: 3px 0 0 3px;  color: #fff', 'background:#7158e2 ; padding: 1px; border-radius: 0 3px 3px 0;  color: #fff', 'background:transparent');
};
...
let RunningCodeErrorSource = /*#__PURE__*/function (RunningCodeErrorSource) {
  RunningCodeErrorSource["errorCenter"] = "errorCenter";
  RunningCodeErrorSource["tryCatch"] = "tryCatch";
  return RunningCodeErrorSource;
}({});

/** 插件 Apply 的参数 */

/** 插件 的 生命周期 */
let PluginMethod = /*#__PURE__*/function (PluginMethod) {
...
/** 日志等级 */
let LoggerLevel = /*#__PURE__*/function (LoggerLevel) {
...
/** 日志的触发起源地 */
let LoggerLanding = /*#__PURE__*/function (LoggerLanding) {
  ...
  return LoggerLanding;
}({});

/** LoggerInfo 中 key 到 梦想埋点参数 key 的映射 */
const LoggerInfoKeyMap = {
  action: 'p',
  event: 'e',
  userNick: 'n',
  level: 'm1',
...
const log$2 = createLogger('LoggerPlus');
/** 日志发送类 */
class LoggerPlus {
  /** 日志的基础信息 */
  ...
  clone(info, ignoredNull = true) {
    ...
    const obj = {
      ...this._publicInfo,
      logInterface: this._logNetworkInterface,
      level: this._curLoggerLevel
    };
    // 覆盖上去
   ...
   * @param args 参数
   */
  _request(level, event, ...args) {
    if (!this.isCanLog(level)) {
      log$2.debug('忽略日志：', level, event, ...args);
      return;
    }
  ...
  encode(obj) {
    ...
    return Object.keys(res).map(key => `${key}=${obj[key]}`).join('&');
  }
...
function coverLoggerInfoToRequestParam(info) {
  return Object.keys(info).reduce((res, k) => {
    res[LoggerInfoKeyMap[k]] = info[k];
    return res;
  }, {});
}

const log$1 = createLogger('LoggerPlugin');
/**
...
class LoggerPlugin extends Plugin {
  ...
  _initParams = {
    appName: "",
    level: LoggerLevel.debug,
    logInterface(_paramsStr, _params, _loggerInfo) {
      throw new Error('未初始化 LoggerPlugin logInterface');
    },
    systemInfo: {},
    userInfo: {
  ...
  /** 修改参数 */
  updatePublicInfo(params, ignoredNull = true) {
  ...
  onError({
    ...
    // 打印错误日志
    $logger.clone({
      landing: LoggerLanding[source]
    }).error(`${error.name}-${error.message}`, JSON.stringify(error));
  }
}

/** 插件的所有方法 */
const PluginMethodList = ['apply', 'onError'];
/** 所有的插件 */
const plugins = {};
...
/** 使用全局事件中心 注册插件的生命周期 */
PluginMethodList.forEach(method => {
...
// 当前执行作用域是否在迭代函数下
var isGeneratorFunction = function (scope) {
  return scope.generator;
};
var isYieldResult = function (scope, value) {
  return isGeneratorFunction(scope) && value === YIELD_SIGNAL;
};
var isReturnResult = function (value) {
  return value === RETURN_SIGNAL;
};
var isContinueResult = function (value) {
  return value === CONTINUE_SIGNAL;
};
var isBreakResult = function (value) {
  return value === BREAK_SIGNAL;
};
/**
 * 是否是函数提升语句
 */
...
var GeneratorStack = /** @class */function () {
  ...
  GeneratorStack.prototype.pushValue = function (value) {
    if (this.running) this.yieldBackValues.push({
      value: value
    });
    this.running = true;
  };
 ...
 * v: 对应的属性名
 */
var RefineForPromoteNameMap = (_a = {}, _a[ForStatement] = 'init', _a[ForInStatement] = 'left', _a[ForOfStatement] = 'left', _a);
/**
...
var evaluate_map = (_b = {}, _b[Program] = function (program, scope) {
  ...
  return indexGeneratorStackDecorate(function (stackData) {
    ...
    for (; stackData.index < nonFunctionList.length; stackData.index++) {
      ...
      if (isYieldResult(scope, result) || isReturnResult(result) || isContinueResult(result) || isBreakResult(result)) return result;
    }
  }, scope);
}, _b[EmptyStatement] = function () {}, _b[ExpressionStatement] = function (node, scope) {
  ...
  for (var new_scope = new Scope("loop" /* ScopeType.Loop */, scope),
    ...
    if (isReturnResult(result)) return result;else if (isContinueResult(result)) continue;else if (isBreakResult(result)) break;
  }
  ...
  return indexGeneratorStackDecorate(function (stackData) {
    ...
    for (; stackData.index < list.length; stackData.index++) {
      ...
      if (id.type === Identifier) {
        ...
        if (isVarPromote && scope.$find(name_1) !== null) continue;
  ...
  return node.elements.map(function (item) {
    return item ? evaluate(item, scope) : null;
  });
  ...
  if (isArrowFunction === void 0) {
    isArrowFunction = false;
  }
  var func;
  var func_name = (node.id || {
    name: "anonymous".concat(anonymousId++)
  }).name;
  if (node.generator) {
    func = function () {
      var args = [];
  ...
  if (isArrowFunction) {
    ObjectDefineProperty(func, "prototype", {
      value: undefined
    });
  }
  // 矫正属性
  ...
  // @ts-ignore
  ObjectDefineProperty(func, "toString", {
    value: function () {
      return thisRunner.source.slice(node.start, node.end);
    },
    configurable: true
  });
  // 矫正name属性
  ...
  if (node.argument.type === Identifier) {
    ...
    if (!$var) throw createError(errorMessageList.notYetDefined, name_7, node, thisRunner.source);
  ...
  if (left.type === Identifier) {
    ...
    if (!$var_or_not) throw createError(errorMessageList.notYetDefined, name_8, node, thisRunner.source);
    $var = $var_or_not;
    ...
    var newProperty_1 = computed ? evaluate(property, scope) : property.name;
    $var = {
      $set: function (value) {
  ...
  return {
    ...
    '**=': function (v) {
      return $var.$set(Math.pow($var.$get(), v)), $var.$get();
    },
    '/=': function (v) {
      return $var.$set($var.$get() / v), $var.$get();
    },
  ...
  return func.apply(this_val, node.arguments.map(function (arg) {
...
for (var kw in keywordTypes) acorn.tokTypes["_" + kw] = keywordTypes[kw];
...
function braceIsBlock(prevType) {
  var parent;
  if (prevType === _colon && (parent = curTokContext()).token == "{") return !parent.isExpr;
  ...
  if (prevType == _braceL) return curTokContext() === b_stat;
  ...
  if (exprList.length) {
    ...
    switch (last.type) {
      ...
      case SpreadElement:
        ...
        if (arg.type !== Identifier && arg.type !== MemberExpression && arg.type !== ArrayPattern) unexpected(arg.start);
        break;
      default:
...
function parseForStatement(node) {
  ...
  if (tokType === _var || tokType === _let) {
    ...
    if ((tokType === _in || options.ecmaVersion >= 6 && isContextual("of")) && init.declarations.length === 1 && !(isLet && init.declarations[0].init)) return parseForIn(node, init);
  ...
  while (!eat(_braceR)) {
    ...
    if (eat(_colon)) {
      ...
      if (isPattern) {
        ...
        if (!refShorthandDefaultPos.start) refShorthandDefaultPos.start = tokStart;
        prop.value = parseMaybeDefault(start, prop.key);
      } else {
        prop.value = prop.key;
      }
...
function parseExprList(close, allowTrailingComma, allowEmpty, refShorthandDefaultPos) {
  ...
  while (!eat(close)) {
    ...
    if (allowEmpty && tokType === _comma) {
      ...
      if (tokType === _ellipsis) elts.push(parseSpread(refShorthandDefaultPos));else elts.push(parseMaybeAssign(false, refShorthandDefaultPos));
    }
  }
  return elts;
}
 ...
 */
var naruseInit = function (params) {
  if (params === void 0) {
    params = {};
  }
...
reactLikeHooksInit({
  getSelfComponent: function () {
    return __SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.NaruseCurrentOwner.current;
  },
...
var isNaruseComponent = function (obj) {
  return obj instanceof NaruseComponent;
};
/**
 * 将函数组件转换为类组件
 */
...
var needPromiseApis = ['getStorage', 'setStorage', 'removeStorage', 'clearStorage', 'getStorageInfo', 'getSystemInfo', 'navigateTo', 'navigateBack', 'setClipboard', 'getClipboard', 'getImageInfo', 'showToast', 'hideToast'];
...
var vnodeDiff = function (newVnode, oldVnode, newParentNode, oldParentNode, path, diffRes) {
  ...
  // key & type
  if (newVnode.key !== oldVnode.key || newVnode.naruseType !== oldVnode.naruseType) {
    res[path] = newVnode;
    return res;
  }
  // naruse-element 单独判断
...
var vnodePropsDiff = function (newVnode, oldVnode, isNaruseComponent) {
  if (isNaruseComponent === void 0) {
    isNaruseComponent = false;
  }
  var res = {};
  // 两者必须都是 naruse 组件
  ...
  for (var newPropKey in newVnode) {
    ...
    if (newPropValue !== oldPropValue) {
      if (newPropKey === 'style' && isObj(newPropValue) && isObj(oldPropValue) && isEmptyObj(vnodePropsDiff(newPropValue, oldPropValue))) {
        continue;
      }
      // 是 NaruseComponent 的前提下都为空的情况下跳过 diff 子元素
      if (isBothNaruseComponent && newPropKey === 'children' && newPropValue && !newPropValue.length && oldPropValue && !oldPropValue.length) {
        continue;
      }
...
var isCustomIdNode = function (node) {
  return !node._uid;
};
var isBaseTypeComponent = function (childVNode) {
  return typeof childVNode == 'string' || typeof childVNode == 'number' || typeof childVNode == 'bigint';
};
...
var createClassElement = function (type, props, childNodes) {
  ...
  return {
    naruseType: "naruse-element" /* VnodeType.NaruseComponent */,
    component: component
  };
};
/**
...
// 放置页面相关的处理
var pageCenter = {};
var ALLOW_EVENT = ['onLoad', 'onShow', 'onReady', 'onHide', 'onUnload', 'onShareAppMessage', 'onTitleClick', 'onOptionMenuClick', 'onPullDownRefresh', 'onTabItemTap', 'onPageScroll', 'onReachBottom'];
/**
...
var withPage = function (component) {
  return (/** @class */function (_super) {
      ...
      class_1.prototype.render = function () {
        ...
        return createElement(component, __assign(__assign({}, this.props), {
...
var cloneElement = function (element, props) {
  ...
  if (isNaruseElement) {
    ...
    var newComponent = __assign(__assign({}, element.component), {
      props: __assign(__assign(__assign({}, oldProps), newProps), {
  ...
  // 基础元素
  return __assign(__assign(__assign({}, element), newProps), {
...
// naruse事件中心
/** 允许继续冒泡的事件 */
var allowPropagetionEventNames = ['onClick', 'onLongClick', 'onTouchStart', 'onTouchMove', 'onTouchEnd', 'onTouchCancel', 'onTransitionEnd', 'onAnimationStart', 'onAnimationIteration', 'onAnimationEnd'];
/**
 * @description 获取节点的路径
 * @author CHC
 ...
 */
var eventNameMap = {};
var methodsTags = ['tap', 'longTap', 'input', 'blur', 'focus', 'load', 'change', 'confirm', 'keyBoardHeightChange', 'scroll', 'scrollToUpper', 'scrollToLower', 'touchStart', 'touchMove', 'touchEnd', 'touchCancel', 'transitionEnd'];
var methodTagTransformMap = {
  'tap': 'click',
  'longTap': 'longClick'
};
var transformFirstApha = function (item) {
  return 'on' + item.slice(0, 1).toLocaleUpperCase() + item.slice(1);
};
// 处理事件映射表
...
var eventCenter = function (event, nodeTree) {
  ...
  if (stopFlag) {
    ...
    eventCenter(__assign(__assign({}, event), {
      target: {
...
var Middware = /** @class */function () {
  function Middware(miniappComponent, NaruseComponentActuator, props) {
    ...
    this.onUpdated = function () {
      if (!_this.naruseComponent) return;
      if (_this.fristRender) _this.naruseComponent.$mounted = true;
  ...
  Middware.prototype.onUnMount = function (isMiniComponentUnmount) {
    if (isMiniComponentUnmount === void 0) {
      isMiniComponentUnmount = false;
    }
...
// naruse模块内容
var Naruse = __assign(__assign(__assign(__assign(__assign(__assign(__assign({}, Hooks), {
 ...
 */
var getNaruseComponentFromProps = function (props) {
 ...
 * @returns {*}
 */
var getNaruseComponentFromCode = function (code, ctx) {
  return __awaiter(void 0, void 0, void 0, function () {
    var naruseConfig, _baseCtx, onRunError, hotImport, baseCtx, $webpack, $$import, context, onError, executeCode, params, exports, component, NaruseComponent_1, compatibleClass;
    return __generator(this, function (_a) {
      if (!code) return [2 /*return*/];
      ...
      $$import = function () {
        ...
        return __awaiter(void 0, void 0, void 0, function () {
          ...
          return __generator(this, function (_a) {
            switch (_a.label) {
              case 0:
                return [4 /*yield*/, hotImport.apply(void 0, args)];
              case 1:
                code = _a.sent();
      ...
      executeCode = function (code) {
        return run(code, context, onError.bind(null, RunningCodeErrorSource.errorCenter));
      };
      params = {
        config: naruseConfig,
        context: context
 ...
 * @param {*} ctx
 */
var renderComponentOnPageWithCode = function (route, code, ctx) {
  return __awaiter(void 0, void 0, void 0, function () {
    var component;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          if (!route) return [2 /*return*/];
  ...
  var naruseBehavior = __assign(__assign({}, getMiniappEventBehavior()), {
    ...
    didMount: function () {
      ...
      if (this.isNaruseMainComponent) {
        var _a = (this.props || {}).unique,
          unique = _a === void 0 ? false : _a;
        // 绑定重新渲染事件
    ...
    didUpdate: function (prevProps) {
      ...
      if (!isEmpty(this.props.component)) {
        var _a = prevProps.component,
          props = _a.props,
          actuator = _a.actuator;
...
export { LoggerLanding, LoggerLevel, LoggerPlugin, LoggerPlus, Naruse, Plugin, createMainBehavior, Naruse as default, naruseExtend, naruseInit, registerPlugin, renderComponentOnPageWithCode };
